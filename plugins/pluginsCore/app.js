...
async function getAppData() {
	if(PLATFORM) {
		switch (PLATFORM) {
			case 'KOYEB': {
				const response = await koyeb.get('/services');
...
async function updateApp(option) {
	...
	if(PLATFORM) {
		switch (PLATFORM) {
			...
			case 'RENDER': {
				const app = await getAppData();
				const updateStatus = await render.post('/services/' + app.id + '/deploys', { option });
...
async function setVar(key, value) {
	if (PLATFORM) {
		switch(PLATFORM) {
			case 'KOYEB': {
				const app = await getAppData();
				const fetchDeploymentData = await koyeb.get('/deployments/' + app.latest_deployment_id);
				...
				const setVariable = await koyeb.patch(`/services/${app.id}`, { definition: { ...fetchDeploymentData.data.deployment.definition, env: envVars }});
				return setVariable.status === 200 ? true : false;
				break;
			}
			case 'RENDER': {
				const app = await getAppData();
				const getVars = await render.get('/services/' + app.id + '/env-vars');
				...
				const setVariable = await render.put('/services/' + app.id + '/env-vars', envVars);
				await updateApp('do_not_clear');
				return setVariable.status === 200 ? true : false;
				break;
			}
			default: {
	...
	if (PLATFORM) {
		switch (PLATFORM) {
			case 'KOYEB': {
				...
				const delVariable = await koyeb.patch(`/services/${app.id}`, { definition: { ...fetchDeploymentData.data.deployment.definition, env: envVars }});
			...
			case 'RENDER': {
				...
				const delVariable = await render.put('/services/' + app.id + '/env-vars', envVars);
				await updateApp('do_not_clear');
				return delVariable.status === 200 ? true : false;
				break;
			}
