...
const {data} = await axios.get(`https://gist.github.com/ESWIN-SPERKY/${config.SESSION_ID.split(':')[1]}/raw`);
fs.writeFileSync('./lib/session/creds.json', JSON.stringify(data));
...
client.ev.on('connection.update', async ({connection, lastDisconnect}) => {
	if(connection === 'connecting') console.log('Connecting...');
		...
		fs.readdirSync('./plugins').filter(file => path.extname(file) === '.js').forEach(file => require(`./plugins/${file}`));
	        var startupMessage = `*X BOT MD STARTED!*\n\n_Mode: ${config.WORK_TYPE}_\n_Prefix: ${config.HANDLERS}_\n_Version: ${config.VERSION}_\n_Menu Type: ${config.MENU_TYPE}_\n_Language: ${config.LANGUAGE}_\n\n*Extra Configurations*\n\n\`\`\`Always online: ${config.ALWAYS_ONLINE ? 'âœ…' : 'âŒ'}\nAuto status view: ${config.AUTO_STATUS_VIEW ? 'âœ…' : 'âŒ'}\nAuto reject calls: ${config.REJECT_CALLS ? 'âœ…' : 'âŒ'}\nAuto read messages: ${config.READ_MESSAGES ? 'âœ…' : 'âŒ'}\nAuto call blocker: ${config.CALL_BLOCK ? 'âœ…' : 'âŒ'}\nAuto status save: ${config.SAVE_STATUS ? 'âœ…' : 'âŒ'}\nAuto status reply: ${config.STATUS_REPLY ? 'âœ…' : 'âŒ'}\nAuto status reaction: ${config.STATUS_REACTION ? 'âœ…' : 'âŒ'}\nLogs: ${config.LOGS ? 'âœ…' : 'âŒ'}\nPM Blocker: ${config.PM_BLOCK ? 'âœ…' : 'âŒ'}\nPM Disabler: ${config.DISABLE_PM ? 'âœ…' : 'âŒ'}\`\`\``;
